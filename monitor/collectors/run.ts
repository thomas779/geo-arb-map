...
import { collectBluesky, type BlueskySource } from './bluesky';
...
const IMPLEMENTED_ADAPTERS = new Set(['rss', 'telegram_html', 'x_search', 'bluesky', 'bluesky_search']);
...
export async function runCollectors(
  ...
  const active = manifest.sources.filter(source =>
    ...
    && IMPLEMENTED_ADAPTERS.has(source.adapter)
  ...
  if (options.sourceId && active.length === 0) {
    throw new Error(`No active, implemented source found with id "${options.sourceId}"`);
  }
    ...
    try {
      ...
      const found = options.fixtureDir
        ...
        : source.adapter === 'x_search'
          ...
          : (source.adapter === 'bluesky' || source.adapter === 'bluesky_search')
            ? await collectBluesky(source as BlueskySource, { retrievedAt })
            : isRssSource(source)
  ...
  // A single failing feed should not sink the run; strict mode still fails on a
  // hard error or a total wipeout so CI surfaces a broken collector.
